...
>
> **Not done — and deliberately not attempted this pass:** converting the remaining ~58
> handlers — the import/export handlers are the natural next slice — but each one needs the same
> "does the real renderer payload actually match the frozen schema, and does the targeted test
> suite still pass" check that caught the `audit.list` and
...
> now exists, unit-tested, and proven end-to-end on 47 commands across eight files:
...
> and `applicationHandlers.ts` (all 10). This also required landing a small, scoped slice of item
> 8 early: `packages/protocol` is now a real linked npm workspace dependency
> (`"@bifurc/protocol": "*"`), which is what makes its Zod schemas importable from `src/` at all.
> See work item 7's own status note for the full detail, the two real schema-gap findings
> (`audit.list`, `runner.saveConfig`), and why the remaining ~58 handlers are next but not yet
> done.
...
unit-tested, and proven on 47 commands across `coreHandlers.ts` (`config.get`, `env.setActive`,
...
`soapHandlers.ts` (2 of 8), `grpcHandlers.ts` (5 of 8), and `applicationHandlers.ts` (all 10);
converting `audit.list` and
...
real functionality — see work item 7's status note. The remaining ~58 handlers — most
