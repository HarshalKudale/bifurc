...
>
> **Not done — and deliberately not attempted this pass:** converting the remaining ~81
> handlers. The straightforward 1:1 channels (`tlsHandlers.ts`, `runnerHandlers.ts`,
> `graphqlHandlers.ts`, `soapHandlers.ts`, `grpcHandlers.ts`, `applicationHandlers.ts`, the
> import/export handlers) are genuinely mechanical and are the natural next slice — but each one
> needs the same "does the real renderer payload actually match the frozen schema" check that
> caught the `audit.list` gap, not a blind conversion. The CRUD channels generated by
> `entityCrudFactory.ts` (`mock:add`, `rule:update`, `ws:delete`, and ~33 more) are a separate,
> larger problem: they are one factory generating N per-kind channels today, while the protocol
> collapses all of them into six generic `entity.*` commands (P1 item
...
> now exists, unit-tested, and proven end-to-end on 23 commands across three files:
...
> gap, and the already-SPLIT `audit:export`), and `folderHandlers.ts` (all 4). This also required
> landing a small, scoped slice of item 8 early: `packages/protocol` is now a real linked npm
> workspace dependency (`"@bifurc/protocol": "*"`), which is what makes its Zod schemas
> importable from `src/` at all. See work item 7's own status note for the full detail, the
> `audit.list` gap, and why the remaining ~81 handlers are next but not yet done.
...
unit-tested, and proven on 23 commands across `coreHandlers.ts` (`config.get`, `env.setActive`,
`workspace.setActive`), `syncHandlers.ts` (16 of 19 channels), and `folderHandlers.ts` (all 4);
converting `audit.list` exposed a real gap between the frozen protocol schema and actual renderer
usage (`fromTs`/`toTs`), so it was deliberately left unconverted rather than silently breaking
date-range filtering — see work item 7's status note. The remaining ~81 handlers — most
mechanically, the `entityCrudFactory.ts` CRUD channels only after a dedicated collapsing pass,
and each needing the same real-usage check that caught the `audit.list` gap — are not yet
