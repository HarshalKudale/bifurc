...
>
> **Not done — and deliberately not attempted this pass:** converting the remaining ~100
> handlers. The straightforward 1:1 channels (most of `coreHandlers.ts`, `syncHandlers.ts`,
> `tlsHandlers.ts`, etc.) are genuinely mechanical and are the natural next slice. The CRUD
> channels generated by `entityCrudFactory.ts` (`mock:add`, `rule:update`, `ws:delete`, and ~33
> more) are not: they are one factory generating N per-kind channels today, while the protocol
> collapses all of them into six generic `entity.*` commands (P1 item 2) — wiring those through
> the registry means collapsing the factory first, a data-modelling change in its own right, not
> a mechanical find-and-replace. Doing that carelessly under session pressure is exactly the
> "missed event" / "duplicate delivery" risk class this document's own risk table warns about for
> item 2 — the same discipline applies here. Recommended next slice for a future session: (a)
> convert the remaining non-CRUD, non-SPLIT handlers one file at a time (mirrors this session's
> proof), then (b) tackle the CRUD collapse as its own dedicated, separately-verified step.
...
> now exists, unit-tested, and proven end-to-end on three `coreHandlers.ts` commands
> (`config.get`, `env.setActive`, `workspace.setActive`). This also required landing a small,
> scoped slice of item 8 early: `packages/protocol` is now a real linked npm workspace dependency
> (`"@bifurc/protocol": "*"`), which is what makes its Zod schemas importable from `src/` at all.
> See work item 7's own status note for the full detail and for why the remaining ~100 handlers
> are next but not yet done.
...
unit-tested, and proven on three `coreHandlers.ts` commands (`config.get`, `env.setActive`,
`workspace.setActive`); the remaining ~100 handlers — most mechanically, the `entityCrudFactory.ts`
CRUD channels only after a dedicated collapsing pass — are not yet converted (see work item 7's
status note for the reasoning). Work item 8 (the physical `packages/*` restructuring + dependency
split) is **mostly not started**, but is no longer untouched: `packages/protocol` is now a real
linked npm workspace (`"workspaces": ["packages/*"]`, `"@bifurc/protocol": "*"` as a dependency),
built with its own `tsup` pipeline, and consumed by `src/commands/registry.ts` — the first real
inter-package dependency in the programme. `packages/engine` itself, and the corresponding
`src/*` → `packages/engine/src/*` file moves, are still not started. See the "Cleanup_plan.md
status" section of `plan/README.md` for the still-open D6 sub-items that also block a complete
P2.
