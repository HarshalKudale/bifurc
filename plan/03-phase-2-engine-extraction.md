...
>
> **Not done — and deliberately not attempted this pass:** converting the remaining ~77
> handlers. The straightforward 1:1 channels (`graphqlHandlers.ts`, `soapHandlers.ts`,
> `grpcHandlers.ts`, `applicationHandlers.ts`, the import/export handlers) are genuinely
> mechanical and are the natural next slice — but each one needs the same "does the real renderer
> payload actually match the frozen schema, and does the targeted test suite still pass" check
> that caught the `audit.list` and `runner.saveConfig` gaps, not a blind conversion. The CRUD
...
> checking each against its real renderer call sites and its targeted test suite the way this
> session did; then (b) tackle the CRUD collapse as its own dedicated, separately-verified step.
...
> now exists, unit-tested, and proven end-to-end on 28 commands across five files:
> `coreHandlers.ts` (`config.get`, `env.setActive`, `workspace.setActive`), `syncHandlers.ts`
> (16 of 19), `folderHandlers.ts` (all 4), `tlsHandlers.ts` (3 of 6), and `runnerHandlers.ts`
> (2 of 6). This also required landing a small, scoped slice of item 8 early: `packages/protocol`
> is now a real linked npm workspace dependency (`"@bifurc/protocol": "*"`), which is what makes
> its Zod schemas importable from `src/` at all. See work item 7's own status note for the full
> detail, the two real schema-gap findings (`audit.list`, `runner.saveConfig`), and why the
> remaining ~77 handlers are next but not yet done.
...
unit-tested, and proven on 28 commands across `coreHandlers.ts` (`config.get`, `env.setActive`,
`workspace.setActive`), `syncHandlers.ts` (16 of 19), `folderHandlers.ts` (all 4),
`tlsHandlers.ts` (3 of 6), and `runnerHandlers.ts` (2 of 6); converting `audit.list` and
`runner.saveConfig` each exposed a real gap between the frozen protocol schema and actual
handler/renderer usage, so both were deliberately left unconverted rather than silently breaking
real functionality — see work item 7's status note. The remaining ~77 handlers — most
mechanically, the `entityCrudFactory.ts` CRUD channels only after a dedicated collapsing pass,
and each needing the same real-usage check that caught those two gaps — are not yet
