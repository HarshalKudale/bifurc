...
>
> **Second batch (this session): `syncHandlers.ts`.** All 19 handlers in that file are now
> routed through the registry except two: `sync:setRemote/disconnect/push/pull/getState/
> setAutoSync/getEntityStatus`, `git:diff/discard/sync/history`, `entity:publish/restore`,
> `folder:publish`, `audit:diff`, and `history:list/diff` (16 of 19) all convert cleanly —
> clean 1:1 channels, no `entityCrudFactory` involvement, exactly the "next slice" called for
> above. **`audit:list` was deliberately reverted to its original direct handler** after
> converting it exposed a real, pre-existing gap: the frozen `AuditListParams` schema is
> `.strict()` and does not declare `fromTs`/`toTs`, but `AuditLogPanel.tsx` actively sends both
> when a date-range filter is set. Routing it through `registry.invoke()` as specified would have
> made every date-filtered audit query throw — a genuine regression, not a registration-only
> change, so it was left as-is with a comment explaining why (a `protocol-changes.md` entry is
> the right fix, not a workaround here). `audit:export` remains untouched for the SPLIT reason
> already documented (engine renders content; shell owns the save dialog).  This is the first
> real evidence for why work item 7's own status note insists on converting "one file at a time"
> rather than batching — the mechanical cases and the one-schema-gap case were sitting in the
> same file.
>
> **Not done — and deliberately not attempted this pass:** converting the remaining ~85
> handlers. The straightforward 1:1 channels (`tlsHandlers.ts`, `folderHandlers.ts`,
> `runnerHandlers.ts`, `graphqlHandlers.ts`, `soapHandlers.ts`, `grpcHandlers.ts`,
> `applicationHandlers.ts`, the import/export handlers) are genuinely mechanical and are the
> natural next slice — but each one needs the same "does the real renderer payload actually match
> the frozen schema" check that caught the `audit.list` gap, not a blind conversion. The CRUD
> channels generated by `entityCrudFactory.ts` (`mock:add`, `rule:update`, `ws:delete`, and ~33
> more) are a separate, larger problem: they are one factory generating N per-kind channels
...
> change in its own right, not a mechanical find-and-replace. Recommended next slice for a future
> session: (a) keep converting the remaining non-CRUD, non-SPLIT handler files one at a time,
> checking each against its real renderer call sites the way this session did for `audit.list`;
> then (b) tackle the CRUD collapse as its own dedicated, separately-verified step.
...
> now exists, unit-tested, and proven end-to-end on 19 commands across two files:
> `coreHandlers.ts` (`config.get`, `env.setActive`, `workspace.setActive`) and `syncHandlers.ts`
> (16 of its 19 channels — everything except `audit:list`, which surfaced a real protocol schema
> gap, and the already-SPLIT `audit:export`). This also required landing a small, scoped slice of
> item 8 early: `packages/protocol` is now a real linked npm workspace dependency
> (`"@bifurc/protocol": "*"`), which is what makes its Zod schemas importable from `src/` at all.
> See work item 7's own status note for the full detail, the `audit.list` gap, and why the
> remaining ~85 handlers are next but not yet done.
...
unit-tested, and proven on 19 commands across `coreHandlers.ts` (`config.get`, `env.setActive`,
`workspace.setActive`) and `syncHandlers.ts` (16 of 19 channels); converting `audit.list` exposed
a real gap between the frozen protocol schema and actual renderer usage (`fromTs`/`toTs`), so it
was deliberately left unconverted rather than silently breaking date-range filtering — see work
item 7's status note. The remaining ~85 handlers — most mechanically, the `entityCrudFactory.ts`
CRUD channels only after a dedicated collapsing pass, and each needing the same real-usage check
that caught the `audit.list` gap — are not yet converted. Work item 8 (the physical `packages/*`
