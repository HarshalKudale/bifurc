...
function parse(content: string): { webhooks: SavedWebhook[]; folders: Folder[] } {
...
export function preflight(wsId: string, source: ImportSource): PreflightResult {
  try {
    const { webhooks } = parse(source.content);
    ...
    return { ok: true, itemCount: webhooks.length, collisionIds };
  } catch (err) {
...
export async function run(
  ...
  try {
    const { webhooks, folders } = parse(source.content);
