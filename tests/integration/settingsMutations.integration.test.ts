...
import { stopCompanionServer, getCompanionPort } from "@bifurc/engine/transport/legacyCompanion";
...
describe("settings — changing the companion port moves the WebSocket server", () => {
  ...
  it("moves the companion server to the newly configured port", async () => {
    ...
    const { startCompanionServer } = await import("@bifurc/engine/transport/legacyCompanion");
